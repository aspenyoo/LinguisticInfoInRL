...
% is test phase perfomrance better than chance?
[h,p,ci,stats] = ttest(data_pcMat-1/3);
stats.tstat
p
...
%% comparing alpha_c across both models 
...
xb = [xb1(:,2) xb2(:,1)];
[h,p,~,stats] = ttest2(xb(:,1),xb(:,2));
fprintf('mean 1: %2.4f, %2.4f \n', mean(xb(:,1)), std(xb(:,1))/sqrt(size(xb,1)))
fprintf('mean 2: %2.4f, %2.4f \n', mean(xb(:,2)), std(xb(:,2))/sqrt(size(xb,1)))
fprintf('rl3wm \alpha_c vs rlwm_dn \alpha: t(%d) = %2.3f, p=%0.3f \n',stats.df, stats.tstat,p)
...
clear all
nsimSubjs = 50; % how many simulated participants we want
